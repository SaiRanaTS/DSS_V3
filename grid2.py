...
def vector_calc(x1, y1, vo, theata, t):

    v = vo * 30.8667
    #print(theata)
    if theata >=0 and theata < 90:
        theata_n = 90-theata
        v = v
    elif theata >=90 and theata < 180:
        theata_n = -(theata - 90)
        v = v
    elif theata >= 180 and theata < 270:
        theata_n = (270-theata)
        v = -v
    else:
        theata_n = 180-(theata-270)
        v = v

    x2 = x1 + (v* math.cos(math.radians(theata_n))*t)
    y2 = y1 + (v* math.sin(math.radians(theata_n))*t)
...
def update(frame):
    ...
    #xo = np.random.randn(60)
    #yo = np.random.randn(60)
    #zo = [np.random.random() for _ in range(60)]

    #cm = plt.cm.get_cmap('RdYlBu_r')
    #ppo =plt.scatter(xo, yo, s=20, c=zo, cmap=cm)
    ...
    #Vector Calculation
    ov_VecX = vector_calc(ov_x[-1], ov_y[-1], ov_spd[-1], ov_heading[-1], 3)
    ts1_VecX = vector_calc(tv1_x[-1],tv1_y[-1],tv1_spd[-1],tv1_heading[-1],3)
    ts2_VecX = vector_calc(tv2_x[-1], tv2_y[-1], tv2_spd[-1], tv2_heading[-1], 3)
    ts3_VecX = vector_calc(tv3_x[-1], tv3_y[-1], tv3_spd[-1], tv3_heading[-1], 3)
    ts4_VecX = vector_calc(tv4_x[-1], tv4_y[-1], tv4_spd[-1], tv4_heading[-1], 3)
    ts5_VecX = vector_calc(tv5_x[-1], tv5_y[-1], tv5_spd[-1], tv5_heading[-1], 3)

    print(ov_heading[-1])

    ax1.plot([ov_x[-1], ov_VecX[0]], [ov_y[-1], ov_VecX[1]], 'k-', linewidth=0.5)
    ax1.plot([tv1_x[-1],ts1_VecX[0]], [tv1_y[-1], ts1_VecX[1]], 'k-',linewidth=0.5)
    ax1.plot([tv2_x[-1], ts2_VecX[0]], [tv2_y[-1], ts2_VecX[1]], 'k-',linewidth=0.5)
    ax1.plot([tv3_x[-1], ts3_VecX[0]], [tv3_y[-1], ts3_VecX[1]], 'k-',linewidth=0.5)
    ax1.plot([tv4_x[-1], ts4_VecX[0]], [tv4_y[-1], ts4_VecX[1]], 'k-', linewidth=0.5)
    ax1.plot([tv5_x[-1], ts5_VecX[0]], [tv5_y[-1], ts5_VecX[1]], 'k-', linewidth=0.5)
